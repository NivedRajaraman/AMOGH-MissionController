...
public class BuoyTask extends MissionController {
	...
	public void setStartDepth(int sD){
		startDepth = sD;
	...
	private void hardHitBuoy() throws InterruptedException{
		heading.update();
		int hHitHeading = heading.getUpdatedValue() + 3;
		MissionController.logToFile(true, "Hardhitting "+color+" buoy at heading : " + hHitHeading );
		...
		while( currentTime <= startTime + 9.5 * 1000 ){
			Sway.At(hHitHeading);
			Surge.At(hHitHeading);			
			Heave.At(targetDepth+10);
		...
		while( currentTime <= startTime + 3 * 1000 ){		// 7 seconds => slightly less for stabilization algo, add 2 more seconds : 5 seconds => without stabilization  
			Sway.At(hHitHeading);
			Surge.At(hHitHeading);			
			Heave.At(targetDepth+10);
			currentTime = System.currentTimeMillis();
			MissionController.logToFile(false, "\rHardhit : Moving Backward, Depth : " + targetDepth+", Target Heading: "+hHitHeading+", Current Heading: "+heading.getUpdatedValue() );
			Thread.sleep(20);
		}
	...
	private void calculate(int targetHeading) throws InterruptedException{
		...
		float reverseSwayThrustMultiplier = 1.25f; //0.80
		float forwardThrustMultiplier = -0.85f;    //0.65
	...
	private void detect(int targetHeading) throws InterruptedException{
		...
		MissionController.logToFile(true, "TargetHeading: "+targetHeading+", TargetDepth: "+targetDepth+", Current Depth: " + depth.getUpdatedValue() + ", TargetSwaySpeed: " + targetSwaySpeed);
		MissionController.logToFile(true, "");
	}
	...
	public void hit(int _sHeading) throws InterruptedException, FileNotFoundException {
		...
		while( !taskComplete ){
			MissionController.logToFile(true, "Hit "+color+" Buoy : Task Has Been Started ");
				  ...
				  int fixedDepth = targetDepth;
					...
					while(!hardHit){
						...
						if(radius == 0 && theta == 0){
							//correctScope(startHeading);
							heading.update();
							correctScope(heading.getUpdatedValue());
